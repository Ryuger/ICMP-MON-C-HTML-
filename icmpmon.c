...
static void serve_host_page(SOCKET c, int id){
    ...
    const char* html =
        ...
        "<label><input id=pauseHover type=checkbox checked> pause on hover</label>"
        "<span id=winInfo></span>"
        "</div><canvas id=cv width=1400 height=460></canvas></div></div><div id=tip></div>"
        "<script>"
        "const id=new URLSearchParams(location.search).get('id');let all=[];let view={live:true,start:0,end:0};let drag=null;let hoverPause=false;"
        ...
        "function draw(j){let cv=document.getElementById('cv');let g=cv.getContext('2d');g.clearRect(0,0,cv.width,cv.height);all=j.samples||[];if(!all.length){g.fillStyle='#666';g.fillText('No history yet. Wait for new samples...',20,30);return;}let d=byRange(all);if(!d.length) d=[all[all.length-1]];let pL=60,pR=20,pT=20,pB=34,W=cv.width,H=cv.height,plotW=W-pL-pR,plotH=H-pT-pB;"
        "let tMin=d[0].ts,tMax=d[d.length-1].ts;if(tMin===tMax)tMax=tMin+1;let max=1;for(let x of d){if(x.rtt>=0&&x.rtt>max)max=x.rtt;}"
        "g.strokeStyle='#eee';for(let i=0;i<=5;i++){let y=pT+i*(plotH/5);g.beginPath();g.moveTo(pL,y);g.lineTo(W-pR,y);g.stroke();let val=Math.round(max*(1-i/5));g.fillStyle='#666';g.fillText(val+' ms',6,y+4);}"
        "g.strokeStyle='#ddd';g.beginPath();g.moveTo(pL,pT);g.lineTo(pL,H-pB);g.lineTo(W-pR,H-pB);g.stroke();"
        "let xAt=t=>pL+((t-tMin)/(tMax-tMin))*plotW;let yAt=v=>H-pB-((v/max)*plotH);let pts=[];let prev=null;let step=Math.max(1,Math.ceil(d.length/1800));"
        "for(let i=0;i<d.length;i+=step){let s=d[i];let x=xAt(s.ts);if(s.rtt>=0){let y=yAt(s.rtt);pts.push({x,y,ts:s.ts,rtt:s.rtt,st:'UP'});if(prev){g.strokeStyle='#0b65d8';g.beginPath();g.moveTo(prev.x,prev.y);g.lineTo(x,y);g.stroke();}g.fillStyle='#1e6bd6';g.beginPath();g.arc(x,y,1.8,0,Math.PI*2);g.fill();prev={x,y};}else{let y=pT+10;pts.push({x,y,ts:s.ts,rtt:-1,st:'LOSS'});g.strokeStyle='#c21807';g.beginPath();g.moveTo(x,y-6);g.lineTo(x,y+10);g.stroke();prev=null;}}"
        "window._pts=pts;"
        ...
        "document.getElementById('winInfo').textContent=`${fmt(tMin)} → ${fmt(tMax)} | points: ${d.length}`;"
        "}"
        "function bindCanvas(){let cv=document.getElementById('cv'),tip=document.getElementById('tip');"
        "cv.onmouseenter=()=>{hoverPause=document.getElementById('pauseHover').checked; if(hoverPause)setLive(false);};"
        "cv.onmouseleave=()=>{tip.style.display='none'; if(hoverPause)setLive(true);};"
        "cv.onmousedown=(e)=>{setLive(false);drag={x:e.clientX,start:view.start,end:view.end};};"
        "window.onmouseup=()=>drag=null;"
        "window.onmousemove=(e)=>{if(drag){let dx=e.clientX-drag.x;let span=Math.max(1,drag.end-drag.start);let dt=Math.round(-dx*(span/Math.max(1,cv.clientWidth)));view.start=drag.start+dt;view.end=drag.end+dt;draw(window._lastJ||{samples:all});return;}let pts=window._pts||[];if(!pts.length)return;let r=cv.getBoundingClientRect();let x=(e.clientX-r.left)*(cv.width/r.width),y=(e.clientY-r.top)*(cv.height/r.height);let best=null,bd=1e9;for(let p of pts){let d=(p.x-x)*(p.x-x)+(p.y-y)*(p.y-y);if(d<bd){bd=d;best=p;}}if(!best||bd>250){tip.style.display='none';return;}tip.style.display='block';tip.style.left=(e.clientX+14)+'px';tip.style.top=(e.clientY+14)+'px';tip.innerHTML=`${fmt(best.ts)}<br>Status: ${best.st}<br>RTT: ${best.rtt<0?'loss':best.rtt+' ms'}`;};"
        "cv.onwheel=(e)=>{e.preventDefault();setLive(false);let k=e.deltaY>0?1.2:0.82;let mid=(view.start+view.end)/2;let span=Math.max(2,(view.end-view.start)*k);view.start=Math.round(mid-span/2);view.end=Math.round(mid+span/2);draw(window._lastJ||{samples:all});};"
        "}"
        "function bindToolbar(){document.getElementById('range').onchange=()=>{setLive(true);go();};document.getElementById('liveBtn').onclick=()=>{setLive(!view.live);if(view.live)go();};"
        "document.getElementById('leftBtn').onclick=()=>{setLive(false);let s=(view.end-view.start)||300;view.start-=Math.round(s*0.25);view.end-=Math.round(s*0.25);draw(window._lastJ||{samples:all});};"
        "document.getElementById('rightBtn').onclick=()=>{setLive(false);let s=(view.end-view.start)||300;view.start+=Math.round(s*0.25);view.end+=Math.round(s*0.25);draw(window._lastJ||{samples:all});};"
        "document.getElementById('zoomInBtn').onclick=()=>{setLive(false);let m=(view.start+view.end)/2,s=Math.max(2,(view.end-view.start)*0.7);view.start=Math.round(m-s/2);view.end=Math.round(m+s/2);draw(window._lastJ||{samples:all});};"
        "document.getElementById('zoomOutBtn').onclick=()=>{setLive(false);let m=(view.start+view.end)/2,s=Math.max(2,(view.end-view.start)*1.4);view.start=Math.round(m-s/2);view.end=Math.round(m+s/2);draw(window._lastJ||{samples:all});};"
        ...
        "bindCanvas();bindToolbar();go();setInterval(()=>{if(view.live)go();},2000);"
        "</script></body></html>";
    http_reply(c, "text/html; charset=utf-8", html);
}
